...
const GestureController: React.FC = () => {
  ...
  const [status, setStatus] = useState<"init" | "ready" | "error" | "denied">("init");
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
          try {
            ...
            setStatus("denied");
            // Do NOT throw error here, just gracefully fallback to 'denied' state
          }
        } else {
    ...
    const predictWebcam = async () => {
      // Stop loop if camera was denied or errored
...
const Polaroid: React.FC<{ data: PhotoData; isFocused: boolean }> = ({ data, isFocused }) => {
  ...
  useFrame((state, delta) => {
    ...
    // Raycasting with hand (normalized mouse pos style)
